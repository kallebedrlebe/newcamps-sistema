...
from automation.fgts.consulta_saldo import consultar_saldo
from automation.fgts.extrato import obter_extrato


def enfileirar_tarefa_fgts(db: Session, bg: BackgroundTasks, data: TarefaCreate) -> Tarefa:
...
def _executar(tarefa_id: int, data: TarefaCreate) -> None:
    ...
    try:
        if data.tipo == TipoTarefa.FGTS_SALDO:
            resultado = consultar_saldo(data.empresa_id)
        elif data.tipo == TipoTarefa.FGTS_EXTRATO:
            resultado = obter_extrato(data.empresa_id)
        else:
            resultado = {}
        registrar_log(tarefa_id, "INFO", "Concluído com sucesso")
        atualizar_status(tarefa_id, StatusTarefa.CONCLUIDA, resultado)
    except Exception as exc:
        logger.exception(f"Erro na tarefa FGTS {tarefa_id}")
